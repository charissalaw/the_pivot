...
RSpec.feature "AdminViewsCompletedOrders", type: :feature do
  scenario "they see only completed orders" do
    ...
    project3 = create(:project, name: "monsters", borrower_id: borrower3.id, category_id: category1.id, country_id: country1.id, status: "deactivated")
    ...
    expect(page).to have_content(project1.borrower.user.fullname)
    expect(page).to have_content(project1.name)
    expect(page).to have_content(project2.borrower.user.fullname)
    expect(page).to have_content(project2.name)

    expect(page).to_not have_content(project3.borrower.user.fullname)
